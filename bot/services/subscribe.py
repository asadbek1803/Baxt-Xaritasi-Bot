...
async def check_channels_after_registration(message: types.Message, bot: Bot):
    ...
    if channels:
        for channel in channels:
            try:
                member = await bot.get_chat_member(chat_id=channel.telegram_id, user_id=user_id)
...
async def send_subscription_message_after_registration(message: types.Message, channels: list):
    ...
    for channel in channels:
        button_text = f"📢 {channel.name}" if not channel.is_private else f"🔒 {channel.name}"
        keyboard.inline_keyboard.append([InlineKeyboardButton(text=button_text, url=channel.link)])
