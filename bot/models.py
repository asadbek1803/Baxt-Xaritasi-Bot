...
class TelegramUser(models.Model):
    telegram_id = models.CharField(
        max_length=50,
        unique=True,
        help_text="Foydalanuvchining Telegram ID",
    ...
    def get_total_referrals_count(self):
        ...
        # 2-daraja
        level_2 = TelegramUser.objects.filter(invited_by__invited_by=self).count()
    ...
    def get_this_month_referrals(self):
        ...
        return self.referrals.filter(registration_date__gte=start_of_month).count()
...
class ReferralPayment(models.Model):
    ...
    amount = models.DecimalField(
        max_digits=10, decimal_places=2, verbose_name="To'lov miqdori"
    )
...
class Kurslar(models.Model):
    name = models.CharField(
        max_length=500, verbose_name="Kurs nomi", help_text="Kurs uchun nom bering"
    )
    price = models.DecimalField(
        max_digits=10,
        decimal_places=2,
        verbose_name="Kurslar narxi",
        help_text="Kurslar uchun to'lov narxi",
    )
    description = models.TextField(
        verbose_name="Kurs uchun tavsif", help_text="Kurs uchun tavsif bering"
    )
    level = models.CharField(
        max_length=20,
        choices=LEVEL_CHOICES,
    ...
    start_date = models.DateField(
        verbose_name="Boshlanish vaqti", blank=True, null=True
    )
    end_date = models.DateField(verbose_name="Tugash vaqti", blank=True, null=True)
...
class Payments(models.Model):
    user = models.ForeignKey(
        ...
        help_text="To'lov qilgan foydalanuvchi",
    )
    course = models.ForeignKey(
        ...
        related_name="payments",
        verbose_name="Kurs",
        help_text="To'lov qilingan kurs",
        null=True,
        blank=True,
    )
    amount = models.DecimalField(
    ...
    payment_type = models.CharField(
        ...
        default="KONKURS",
        verbose_name="To'lov turi",
        help_text="To'lov turi: Konkurs, Kurs yoki Xayriya",
    )
    status = models.CharField(
        max_length=20,
            ...
            if self.payment_type == "COURSE" and self.course:
                ...
                if self.course.level and self.course.level.startswith("level_"):
                    ...
                    CourseParticipant.objects.get_or_create(
                        user=self.user, course=self.course, defaults={"payment": self}
                    )
...
class MandatoryChannel(models.Model):
    ...
    is_active = models.BooleanField(
        default=True, verbose_name="Faol", help_text="Kanal faolmi"
    )
...
class PrivateChannel(models.Model):
    ...
    name = models.CharField(max_length=255, verbose_name="Kanal nomi")
    telegram_id = models.CharField(
        max_length=100, verbose_name="Telegram ID", help_text="Kanalning Chat ID"
    )
    invite_link = models.URLField(
        verbose_name="Taklif havola", help_text="Kanalga taklif qilish uchun havola"
    )
...
class Notification(models.Model):
    ...
    notification_type = models.CharField(
        max_length=20, choices=NOTIFICATION_TYPES, verbose_name="Bildirish turi"
    )
