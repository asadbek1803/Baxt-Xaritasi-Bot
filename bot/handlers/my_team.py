...
async def safe_edit_message(query: types.CallbackQuery, text: str, keyboard: InlineKeyboardMarkup = None):
...
def create_team_keyboard(page: int, total_pages: int, user_id: str) -> InlineKeyboardMarkup:
    ...
    if page > 0:
        nav_buttons.append(
            InlineKeyboardButton(
                text="◀️ Oldingi", 
                callback_data=f"team_page:{user_id}:{page-1}"
            )
        )
    ...
    action_buttons = [
        [
            InlineKeyboardButton(
                text="🌳 Referal daraxti", 
                callback_data=f"ref_tree:{user_id}"
            ),
            InlineKeyboardButton(
                text="📊 Statistika", 
        ...
        [
            InlineKeyboardButton(
                text="🔄 Yangilash", 
                callback_data=f"team_page:{user_id}:{page}"
            )
        ]
    ]
...
async def show_referral_tree(query: types.CallbackQuery, user_id: str):
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(
                text="◀️ Orqaga", 
                callback_data=f"team_page:{user_id}:0"
            )
        ]
    ])
