...
async def create_referral(callback: types.CallbackQuery, state: FSMContext):
    try:
        ...
        if not root_referrer:
            await callback.answer("❌ Referral tizimida muammo topildi! Admin bilan bog'laning.", show_alert=True)
            return
        ...
        try:
            ...
            if not referral_payment:
                await callback.answer("❌ To'lov so'rovi yaratishda xatolik!", show_alert=True)
                return
        ...
        card_holder = getattr(root_referrer, 'card_holder_full_name', "Ma'lumot mavjud emas")
    ...
    try:
        ...
        try:
            user = await asyncio.wait_for(
                sync_to_async(TelegramUser.objects.get)(telegram_id=str(message.from_user.id)),
                timeout=10
            )
            payment = await asyncio.wait_for(
                sync_to_async(
                    lambda: ReferralPayment.objects.select_related('referrer', 'user').get(id=payment_id)
                )(),
                timeout=10
            )
        except (TelegramUser.DoesNotExist, ReferralPayment.DoesNotExist, asyncio.TimeoutError):
    ...
    try:
        ...
        try:
            payment = await sync_to_async(
                lambda: ReferralPayment.objects.select_related('user').get(id=payment_id)
            )()
            ...
            return await callback.answer("❌ Ma'lumotlar bazasida xatolik!", show_alert=True)
        ...
        try:
            ...
            return await callback.answer("❌ To'lovni saqlashda xatolik!", show_alert=True)
        
        # Delete the admin message0
        global xabar
        if xabar:
            try:
                await xabar.delete()
