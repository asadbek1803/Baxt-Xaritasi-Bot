...
from bot.handlers.credit_card import start_add_card
from core.settings import TELEGRAM_BOT_USERNAME
from bot.models import TelegramUser, ReferralPayment
from bot.selectors import create_referral_payment_request, get_root_referrer, fetch_user
...
async def create_referral(callback: types.CallbackQuery, state: FSMContext):
    try:
        ...
        user = await fetch_user(chat_id=user_id)

        if not user.card_number:
            await start_add_card(callback, state)
            return
...
async def confirm_referral_payment(callback: types.CallbackQuery):
    try:
        ...
        await callback.message.answer("To'lov muvaffaqiyatli tasdiqlandi!")
