...
async def show_card_info(callback: types.CallbackQuery):
    ...
    try:
        ...
        card_holder = (
            user_info.card_holder_full_name
            if user_info.card_holder_full_name
            else "Kiritilmagan"
        )
        ...
        if user_info.card_number and len(user_info.card_number) >= 4:
            masked_card = (
                "*" * (len(user_info.card_number) - 4) + user_info.card_number[-4:]
            )
        else:
...
async def cancel_card_setup(message: types.Message, state: FSMContext):
    ...
    if current_state in [
        CardInfoStates.waiting_for_card_number,
        CardInfoStates.waiting_for_card_holder_name,
    ]:
        await state.clear()
        await message.answer(
